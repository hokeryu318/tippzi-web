...
if (isset($json_data)) {
    ...
    $wallert_insert = mysqli_query($conn, "insert into claim_deal(`deal_id`,`user_id`)
											value('$deal_id','$user_id')");
    $engagement_insert = mysqli_query($conn, "insert into clicks(`bar_id`,`user_id`)
											value('$bar_id','$user_id')");
    ...
    if ($wallert_insert) {
        ...
        if (mysqli_num_rows($result_check_existing_user) > 0) {
            ...
            $get_bar = mysqli_query($conn, "SELECT Id,name,service_name,post_code,address,telephone,website,email,description,music_type,latitude,longitude,category,approve_flag,( 6371 * acos ( cos ( radians('$lat') ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians('$lon') ) + sin ( radians('$lat') ) * sin( radians( latitude ) ) )) AS distance FROM bar WHERE Id = '$bar_id' HAVING distance < 10000000 ORDER BY distance asc");
            //			}
            //			else if ($category_flag == false) {
//
            //			        $get_bar = mysqli_query($conn, "SELECT Id,name,post_code,address,telephone,website,email,description,music_type,latitude,longitude,category,( 6371 * acos ( cos ( radians('$lat') ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians('$lon') ) + sin ( radians('$lat') ) * sin( radians( latitude ) ) )) AS distance FROM bar HAVING distance < 10000000 ORDER BY distance asc");
            //			}
        ...
        if (mysqli_num_rows($check_wallets) > 0) {
            while ($wallet_row = mysqli_fetch_array($check_wallets)) {
                ...
                $get_wallet_bar_res = mysqli_query($conn, "SELECT Id,name,post_code,service_name,address,telephone,website,email,description,music_type,latitude,longitude,category,approve_flag,( 6371 * acos ( cos ( radians('$lat') ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians('$lon') ) + sin ( radians('$lat') ) * sin( radians( latitude ) ) )) AS distance FROM bar where Id='$wallet_bar_id' HAVING distance < 10000000 ORDER BY distance asc");
                    ...
                    if ($difference >= 0) {
                        ...
                        if ($qty_c_value > 0) {
                            ...
                            if (mysqli_num_rows($check_claim) == 0) {
                                ...
                                $wallet_bar['music_type'] = $get_wallet_bar_row['music_type'];
