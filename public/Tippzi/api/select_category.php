...
if (mysqli_num_rows($result_check_existing_user) > 0) {
    $check_category = mysqli_query($conn, "select * from bar where category='$category'");
    if (mysqli_num_rows($check_category) > 0) {
        ...
        $response['bars'] = array();
        $get_bar = mysqli_query($conn, "SELECT Id,name,service_name,post_code,address,telephone,website,email,description,music_type,latitude,longitude,category,( 6371 * acos ( cos ( radians('$lat') ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians('$lon') ) + sin ( radians('$lat') ) * sin( radians( latitude ) ) )) AS distance FROM bar where category='$category' HAVING distance < 10000000 ORDER BY distance asc");
        while ($bar_row = mysqli_fetch_array($get_bar)) {
            ...
            if (mysqli_num_rows($select_deal_bar) > 0) {
        ...
        if (mysqli_num_rows($check_wallets) > 0) {
            while ($wallet_row = mysqli_fetch_array($check_wallets)) {
                ...
                $get_wallet_bar_res = mysqli_query($conn, "SELECT Id,name,service_name,post_code,address,telephone,website,email,description,music_type,latitude,longitude,category,( 6371 * acos ( cos ( radians('$lat') ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians('$lon') ) + sin ( radians('$lat') ) * sin( radians( latitude ) ) )) AS distance FROM bar where Id='$wallet_bar_id' HAVING distance < 10000000 ORDER BY distance asc");
        ...
        $response['success'] = 'false';
        $response['message'] = 'Bar is not exist';
