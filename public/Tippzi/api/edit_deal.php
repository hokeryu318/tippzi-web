...
$deal_bar_update = mysqli_query($conn, "update deal set bar_id='$bar_id',
														title='$title',
														description='$description',
														duration='$duration',
														qty = '$qty' where Id='$deal_id'");
    $response['monday'] = $deal_days['monday'];
    $response['tuesday'] = $deal_days['tuesday'];
    $response['wednesday'] = $deal_days['wednesday'];
    $response['thursday'] = $deal_days['thursday'];
    $response['friday'] = $deal_days['friday'];
    $response['saturday'] = $deal_days['saturday'];
    $response['sunday'] = $deal_days['sunday'];

    if ($deal_bar_update) {
        $check_deal = mysqli_query($conn, "select * from deal_days where deal_id='$deal_id'");
        if (mysqli_num_rows($check_deal) > 0) {
            ...
            $deal_days_update = mysqli_query($conn, "update deal_days set monday='$monday_flag',tuesday='$tuesday_flag',
																		 wednesday='$wednesday_flag',
																		thursday='$thursday_flag',
																		friday='$friday_flag',
																		saturday='$saturday_flag',
																		sunday='$sunday_flag' where deal_id='$deal_id'");
        }
