...
if (mysqli_num_rows($check_wallet) == 0) {
    $wallert_insert = mysqli_query($conn, "insert into in_wallet(`deal_id`,`user_id`,`bar_id`)
								value('$deal_id','$user_id','$bar_id')");
    if ($wallert_insert) {
        ...
        if (mysqli_num_rows($result_check_existing_user) > 0) {
            ...
            $claimed_check = false;
            $response['bars'] = array();
            $get_bar = mysqli_query($conn, "SELECT Id,name,post_code,address,service_name,telephone,website,email,description,music_type,latitude,longitude,category,approve_flag,( 6371 * acos ( cos ( radians('$lat') ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians('$lon') ) + sin ( radians('$lat') ) * sin( radians( latitude ) ) )) AS distance FROM bar WHERE Id = '$bar_id' HAVING distance < 10000000 ORDER BY distance asc");
